...
module.exports = {
    ...
    'httpRequest': {
        ...
        'options': {
            ...
            'should return a string if encoding is set': function(t) {
                t.mockHttp().when('http://some/url').send(200, new Buffer('test response'));
                request({
                    url: "http://some/url",
                    encoding: 'hex'
                    ...
                    t.equal(body, new Buffer('test response').toString('hex'));
                    t.done();
                })
            },

            'should return error if encoding is bad': function(t) {
                t.mockHttp().when('http://some/url').send(200, new Buffer('test response'));
                request({
                    url: "http://some/url",
                    encoding: 'nonesuch'
                    ...
                    t.ok(err);
                    t.contains(err.message, 'Unknown encoding');
                    t.done();
                })
            },
            ...
            'should return without ending call if noReqEnd': function(t) {
                t.mockHttp()
                    .when('http://host/path')
                        .send(200, 'test response')
                ;
